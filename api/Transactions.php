...
class Transactions
{
    ...
    {
        $userId = $_SESSION["user_auth_id"];
        $sql = "SELECT t.TRANSACTION_ID,date_format(str_to_date(t.TRANSACTION_DATE, '%d-%m-%Y'), '%d-%m-%y') as TRANSACTION_DATE ,t.TRANSACTION_REMARK, t.PAY_AMOUNT, t.RECEIVED_AMOUNT,t.TRANSACTION_CREATE_DATE,t.TRANSACTION_UPDATE_DATE,
        ...
        debtors.DEBTOR_NAME,debtors.DEBTOR_MOBILE,debtors.DEBTOR_EMAIL,debtors.DEBTOR_ADDRESS
         ...
         ORDER BY date_format(str_to_date(t.TRANSACTION_DATE, '%d-%m-%Y'), '%Y'),date_format(str_to_date(t.TRANSACTION_DATE, '%d-%m-%Y'), '%m'),date_format(str_to_date(t.TRANSACTION_DATE, '%d-%m-%Y'), '%d') DESC";
        $result = $this->conn->query($sql);
        $rows = $result->fetch_all(MYSQLI_ASSOC);
        $this->conn->close();
        return $rows;
