...
<head>
    ...
    <title>Received - Debtor Book</title>
    <?php 
...
<body>
    ...
    <div class="sidebarContent">
          ...
          <!-- navbar end-->
            ...
            <!-- main content-->
            <div class="main p-5">
                  <h1 class="gray-text"> <i class="fa fa-rupee-sign"></i> Received</h1>
                  <hr/>
                  <form action="" method="post" onsubmit="return validateReceivedForm()">
                     <div class="row">
                         <div class="col-md-12 col-lg-6 col-sm-12">
                             ...
                             <div class="form-group">
                                 ...
                                 <input type="number" name="receivedAmount" id="receivedAmount" class="mb-2" placeholder="Received Amount"  min="0" required>                               
                             </div>
                             ...
                             <!--submit button -->
                          <button type="submit" class="btn-block" name="receivedTransaction"><i class="fa fa-save"></i> Save</button>
                         </div>
                     </div>
                  </form>
            </div> <!---main -------->
...
<script>
   function validateReceivedForm(){
     ...
     if(!paymentDate.match(datePattern)){
        ...
        return false;
     }else if(receivedAmount=== "" || receivedAmount.length === 0 || receivedAmount<=0){
...
<?php 
   if(
       ...
       isset($_POST["receivedAmount"]) &&
       isset($_POST["remark"]) &&
       isset($_POST["receivedTransaction"]) 
     ){
        ...
        if(!ValidationHelper::validateDate($paymentDate)){
                ...
                <?php
        }elseif(empty($debtorId) || empty($receivedAmount) || empty($remark) ){
            //empty debtorid
                ?>
                   <script>
         ...
         $result = $transactions->receivedTransaction($paymentDate,$debtorId,$receivedAmount,$remark);
         if($result){
            //   insert payment transaction successfully
            ?>
            <script>
                  toastr.success('Received Transaction Saved!');
            </script>
           <?php
         }else{
